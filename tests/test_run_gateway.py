...
def test_user_writes_config_then_restart_backend_preserves_it(tmp_path, monkeypatch):
    ...
    重启后端 = 桌面端宿主终止并重新启动 gateway = 重新执行 create_app() →
    ...
    # 3. 用户点「重启后端」→ 桌面端宿主重启 gateway → gateway 重新走 create_app()
...
# ── 桌面端 webview origin（CORS / CSRF）────────────────────────────


def test_configure_gateway_security_injects_dev_origins_only(monkeypatch):
    """开发态 Vite dev server（``http://localhost:1420``）直连 gateway 属跨源，
    必须注入 ``GATEWAY_CORS_ORIGINS``。Electron 打包态渲染层经主进程同源反向
    代理访问 gateway（代理改写 Origin 为 gateway 自身 origin），gateway 视为
    同源，不再需要 tauri:// / app:// 等自定义 scheme origin。"""
    monkeypatch.delenv("GATEWAY_CORS_ORIGINS", raising=False)

    from scripts.run_gateway import _configure_gateway_security

    _configure_gateway_security()
    origins = os.environ["GATEWAY_CORS_ORIGINS"].split(",")
    assert "http://localhost:1420" in origins
    assert "http://127.0.0.1:1420" in origins
    # 打包态由主进程同源代理，不再注入任何 tauri:// / app:// 自定义 scheme origin
    assert not any("tauri" in o or "app://" in o for o in origins)


def test_configure_gateway_security_merges_existing_origins(monkeypatch):
    """用户预设的 GATEWAY_CORS_ORIGINS 不被覆盖，仅追加缺失的 dev origin。"""
    monkeypatch.setenv("GATEWAY_CORS_ORIGINS", "https://custom.example")

    from scripts.run_gateway import _configure_gateway_security

    _configure_gateway_security()
    origins = os.environ["GATEWAY_CORS_ORIGINS"].split(",")
    assert "https://custom.example" in origins
    assert "http://localhost:1420" in origins
...
def test_resolve_app_data_root_respects_env(monkeypatch, tmp_path):
    """KSTOCK_APP_DATA_DIR 显式指定时优先（Electron 宿主 / 调试）。"""
