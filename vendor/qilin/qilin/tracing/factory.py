...
from qilin.tracing.monocle import is_monocle_setup_completed
...
def build_tracing_callbacks() -> list[Any]:
    ...
    if is_monocle_tracing_enabled() and not is_monocle_setup_completed():
        logger.debug(
            "MONOCLE_TRACING is set but Monocle is not initialized in this process — only the Gateway lifespan runs setup automatically; embedded/TUI callers must call qilin.tracing.setup_monocle_tracing_if_enabled() themselves."
        )
