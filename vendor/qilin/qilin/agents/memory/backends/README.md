...
Each subfolder under `agents/memory/backends/` is a pluggable memory backend. Swap the active one by changing one line in `config.yaml` - no qilin core changes required.

- `qilinmem/` - the default backend (qilin's own: structured facts + JSON storage).
...
| File | What to change |
|---|---|
| `backends/<yourname>/config.py` | Declare your config fields + `from_backend_config` (parse `backend_config`; read `storage_path` from it - **do not import qilin path helpers**) |
...
```yaml
memory:
  manager_class: <name>        # qilinmem / noop / <yourname>
  backend_config: { ... }      # that backend's private config
```

Then **restart qilin** - the memory manager is a process-level singleton; a running process does not hot-reload config or backend code.
...
`get_memory` / `export_memory` / `clear_memory` / `import_memory` return a dict that the gateway casts to the **QiLinMem shape** (`MemoryResponse`: `version` / `lastUpdated` / `user` / `history` / `facts[]`). Your backend must return a dict this shape accepts, or:
...
A non-QiLinMem backend maps its native records (e.g. `{"results": [...]}`) into this shape via a small adapter helper.
...
> [!IMPORTANT]
> A backend talks to the host through exactly **two channels**: (1) the ABC method arguments (`manager.py`), and (2) the `backend_config` dict. The **only** `from qilin` import allowed anywhere in your backend folder is the ABC contract line in `<name>_manager.py`:

```python
from qilin.agents.memory.manager import MemoryManager
```

Change that one line (and only that line) to port the backend to another agent. **Do not import qilin path helpers, config singletons, or models** - get `storage_path` and everything else from `backend_config`.
...
Each backend's `from_config` consumes the hooks it needs (QiLinMem does; noop ignores them).
...
| File | Role |
|---|---|
| `packages/harness/qilin/agents/memory/manager.py` | ABC + factory + scanner |
| `packages/harness/qilin/agents/middlewares/memory_middleware.py` | `after_agent` -> `manager.add` |
| `packages/harness/qilin/agents/memory/summarization_hook.py` | summarization -> `manager.add_nowait` |
| `packages/harness/qilin/agents/lead_agent/prompt.py` | `_get_memory_context` -> `manager.get_context` |
| `app/gateway/routers/memory.py` | HTTP endpoints -> `manager.*` (direct call + try/except `NotImplementedError`) |
| `packages/harness/qilin/config/memory_config.py` | shared 4 fields (`enabled` / `injection_enabled` / `manager_class` / `backend_config`) |
| `frontend/src/components/workspace/settings/memory-settings-page.tsx` | frontend memory page (assumes QiLinMem shape) |

> [!NOTE]
> The gateway and frontend are currently hard-coded to the QiLinMem shape - that's why backends must return QiLinMem-shape data (contract #2). Making them fully backend-agnostic is a larger refactor.
...
2. **Return the QiLinMem shape.** Otherwise the frontend crashes with `Invalid time value` and your data is silently dropped. Build a small adapter helper to map your native records into it.
...
4. **Don't import `runtime_home`.** Read `storage_path` from `backend_config`. (The `noop` template shows the correct pattern; importing qilin path helpers breaks portability - contract #4.)
5. **Restart qilin after changes.** The manager is a process-level singleton; a running process does not hot-reload config or backend code.
6. **Cap `get_context` length yourself.** The host applies no token budget; the backend must truncate (QiLinMem has `max_injection_tokens`; noop does not).

## Reference

- **Template**: `noop/` - minimal implementation with full docstrings; copy and go.
- **Contract + factory**: `packages/harness/qilin/agents/memory/manager.py` (`MemoryManager` base, `MemoryCallbacks`, `get_memory_manager` factory).
