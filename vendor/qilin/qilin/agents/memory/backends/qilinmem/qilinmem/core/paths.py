"""QiLinMem's own storage path resolution (no qilin ``get_paths`` / ``AGENT_NAME_PATTERN``).

The host no longer dictates where QiLinMem stores data. Root = ``config.storage_path``
(if set, absolute or relative) or ``$DEERMEM_DATA_DIR`` or ``~/.qilinmem/``.
...
ids) and agent_name validated against an inlined pattern -- QiLinMem does not
...
# agent_name validation (inlined; was qilin's AGENT_NAME_PATTERN).
...
def _default_root() -> Path:
    """QiLinMem's default data root: ``$DEERMEM_DATA_DIR`` or ``~/.qilinmem/``."""
    env = os.environ.get("DEERMEM_DATA_DIR")
    if env:
        return Path(env)
    return Path.home() / ".qilinmem"


def memory_file_path(
    config: QiLinMemConfig,
    agent_name: str | None = None,
    *,
    user_id: str | None = None,
) -> Path:
    """Resolve the memory file path under QiLinMem's own data root.

    ``config.storage_path`` (absolute or relative) is the root. Empty -> default root
    (``$DEERMEM_DATA_DIR`` / ``~/.qilinmem/``). The host (qilin factory)
