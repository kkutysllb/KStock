"""QiLinMem's own LLM construction (no qilin ``create_chat_model``).

``build_llm(model_config)`` builds a langchain ``ChatModel`` from QiLinMem's
model sub-config (provider/model/api_key/base_url/temperature) via
``langchain.chat_models.init_chat_model``. QiLinMem owns the resulting instance
(``self._llm``) and injects it into ``MemoryUpdater`` (dependency injection).

``QiLinMem.__init__`` prefers a host-injected ``host_llm`` (the qilin
...
- standalone QiLinMem then has no LLM (non-LLM ops still work; an update
...
if TYPE_CHECKING:
    from ..config import QiLinMemModelConfig

logger = logging.getLogger(__name__)


def build_llm(model_config: QiLinMemModelConfig | None) -> Any:
    """Build a langchain ChatModel from QiLinMem's model config (DI).
