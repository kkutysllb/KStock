...
QiLin's memory store. Fully stateless in-process: dedup, fact extraction,
...
- Fact CRUD, `import_memory`, and Settings-page memory editing are not
  implemented (gateway returns 501). QiLinMem remains the default backend.
- No migration of existing QiLinMem data.
...
`MemoryManager` contract. QiLin offloads it at every async boundary: the
