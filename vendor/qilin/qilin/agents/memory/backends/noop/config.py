...
rule** (read before writing a backend):
    ...
    It MUST NOT import qilin modules or hardcode qilin paths. The ONLY
    ``from qilin`` line allowed in the whole backend folder is the ABC
...
What the factory (``manager.py::get_memory_manager``) provides to each backend:
  - ``backend_config["storage_path"]`` (str): a writable state dir (the host's
    ...
    storage root** -- do NOT call a qilin path helper yourself.
