...
``backend_config`` dict. The ONLY ``from qilin`` import allowed in this
folder is the ABC contract line below -- change that one line to port the
backend to another agent. Do NOT import qilin path helpers, config
...
Writing a new backend:
  ...
  2. ``config.py``: declare your config knobs + ``from_backend_config`` (parse
     ``backend_config``; read ``storage_path`` from it, NOT from qilin).
...
``clear_memory`` / ``import_memory`` returns to a QiLinMem-shape response
(``version`` / ``lastUpdated`` / ``user`` / ``history`` / ``facts[]``). A real
backend returns a dict castable to that shape (a non-QiLinMem backend maps
...
# ABC contract -- the ONE allowed `from qilin` in this backend folder.
...
def _empty_memory() -> dict[str, Any]:
    ...
    QiLinMem-shape doc (see the return-shape note in the module docstring).
    """
    return {"facts": []}
...
class NoopMemoryManager(MemoryManager):
    ...
    # (see QiLinMem for full implementations):
    #
