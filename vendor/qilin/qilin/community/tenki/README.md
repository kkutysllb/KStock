...
Runs each QiLin sandbox as a [Tenki](https://tenki.cloud) cloud sandbox — an
...
```yaml
sandbox:
  use: qilin.community.tenki:TenkiSandboxProvider
...
```bash
pip install "qilin-harness[tenki]"
```

The `tenki-sandbox` package is an optional QiLin harness extra, not part of
...
API; directory and content search shell out and reuse `qilin.sandbox.search`,
...
QiLin's `/mnt/user-data` virtual prefix is remapped under the writable
