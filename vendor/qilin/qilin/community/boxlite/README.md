...
Runs each QiLin sandbox as a [BoxLite](https://github.com/boxlite-ai/boxlite)
...
[#3439](https://github.com/bytedance/qilin/issues/3439) and
[#3213](https://github.com/bytedance/qilin/issues/3213); discussion in
[#3936](https://github.com/bytedance/qilin/issues/3936).

## Configuration

```yaml
sandbox:
  use: qilin.community.boxlite:BoxliteProvider
...
```bash
pip install "qilin-harness[boxlite]"
```

The `boxlite` package is an optional QiLin harness extra, not part of the
...
hosts, such as Windows, should use another sandbox provider or run QiLin from
a supported Linux/macOS environment.

**Host requirement:** BoxLite boots micro-VMs, so a Linux host needs KVM — i.e.
nested virtualization when QiLin runs inside a cloud VM. macOS uses
...
inside the box and reuse `qilin.sandbox.search`, mirroring `e2b_sandbox`:
