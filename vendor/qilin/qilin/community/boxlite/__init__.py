"""BoxLite micro-VM backend for QiLin sandboxes.
...
macOS) — behind QiLin's :class:`Sandbox` / :class:`SandboxProvider` contract.
Each sandbox is a hardware-isolated VM with its own kernel that runs any OCI
image unchanged. See https://github.com/bytedance/qilin/issues/3936.
...
Install the optional runtime before selecting this provider::
    ...
    pip install "qilin[boxlite]"

Host requirement: BoxLite boots micro-VMs, so a Linux host needs KVM (nested
virtualization when QiLin itself runs inside a cloud VM); macOS uses
