...
class SandboxOwnershipConfig(BaseModel):
    ...
    redis_url: str | None = Field(
        default=None,
        description="Redis URL for the redis ownership type. If omitted, QILIN_SANDBOX_OWNERSHIP_REDIS_URL, QILIN_STREAM_BRIDGE_REDIS_URL, REDIS_URL, or redis://localhost:6379/0 is used.",
    )
...
class VolumeMountConfig(BaseModel):
    ...
    host_path: str = Field(
        ...
        description=(
            ...
            "``qilin-gateway`` container, so the host directory must also be "
...
class SandboxConfig(BaseModel):
    ...
    AioSandboxProvider specific options:
        ...
        container_prefix: Prefix for container names (default: qilin-sandbox)
    ...
    use: str = Field(
        ...,
        description="Class path of the sandbox provider (e.g. qilin.sandbox.local:LocalSandboxProvider)",
    )
