...
The value stored here is QiLin's request-level correlation id. It is
separate from Langfuse's own trace id and from QiLin run ids.
"""
...
QILIN_TRACE_METADATA_KEY: Final[str] = "qilin_trace_id"
_MAX_TRACE_ID_LENGTH: Final[int] = 512

_current_trace_id: Final[ContextVar[str | None]] = ContextVar("qilin_current_trace_id", default=None)
_trace_id_from_request_header: Final[ContextVar[bool]] = ContextVar(
    "qilin_trace_id_from_request_header",
    default=False,
)
...
def resolve_qilin_trace_id(metadata_trace_id: object) -> str | None:
    """Resolve the effective ``qilin_trace_id`` for a run.

    When Gateway ``TraceMiddleware`` bound a valid inbound ``X-Trace-Id``,
    that value wins over ``config.metadata.qilin_trace_id`` so logs,
