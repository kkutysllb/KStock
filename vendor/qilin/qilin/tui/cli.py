"""Command-line entry point and launch-mode planning for the QiLin TUI.
...
headless one-shot. ``main`` wires that decision to the embedded ``QiLinClient``
and lazily imports the Textual app only when actually launching the UI, so the
``qilin`` console script still runs headless commands without Textual present.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="qilin",
        description="QiLin terminal workbench — a TUI over the embedded QiLin harness.",
        add_help=True,
    )
...
def plan_launch(
    ...
    *,
    ...
    if args.cli:
        ...
        return LaunchPlan(
            mode="headless-help",
            reason='--cli needs a message. Try: qilin --print "your question".',
        )

    forced_tui = bool(args.tui)
    if forced_tui or _truthy(env.get("QILIN_TUI")) or (stdin_isatty and stdout_isatty):
        return LaunchPlan(
            mode="tui",
            message=positional,
            thread_id=resume,
...
_HEADLESS_HELP = """\
qilin — QiLin terminal workbench

  qilin                      launch the terminal UI (TTY required)
  qilin --tui                force the terminal UI
  qilin --continue           resume the most recent thread in the UI
  qilin --resume THREAD      resume a thread by id or title
  qilin --print "question"   one-shot answer to stdout
  qilin --json "question"    stream newline-delimited JSON events
  echo "question" | qilin --print
"""
...
def _run_tui(plan: LaunchPlan) -> int:
    try:
        ...
        # `qilin.tui.app` module for the forbidden top-level `app` package.
        from qilin.tui.app import run_tui
    except ModuleNotFoundError as exc:  # textual missing
        if getattr(exc, "name", "") == "textual" or "textual" in str(exc):
            msg = "The terminal UI needs the optional 'textual' dependency.\nInstall it with:  uv pip install 'qilin[tui]'   (or: pip install textual)\n"
            if plan.forced_tui:
