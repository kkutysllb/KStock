...
from qilin.config.agents_config import GitHubTriggerConfig
...
def _mentions(body: str, login: str) -> bool:
    ...
    ``@qilin`` would falsely match ``@qilin-bot`` (a different,
    ...
    character (e.g. ``foo@qilin`` inside an email address) to avoid
...
def event_should_fire(
    ...
    if trigger.require_mention:
        ...
        # Boundary-aware @-mention match: ``@qilin`` must NOT match
        # ``@qilin-bot`` (a distinct, legitimate GitHub login). See
