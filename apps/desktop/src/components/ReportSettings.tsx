...
const REPORT_SKILL_IDS = ["chart-visualization"];
const SKILL_LABELS: Record<string, string> = { "chart-visualization": "图表可视化技能" };
...
export function ReportSettings({ onNavigateToExtensions }: ReportSettingsProps) {
  ...
  return <div className="report-settings">
    ...
    <section className="settings-card" aria-label="报告技能状态">
      <strong>报告技能状态</strong><p className="runtime-config-desc">报告渲染由内置 render_html_report / render_html_report_from_file 工具完成（SVG 内嵌图表）；已落盘 JSON 会优先按文件路径渲染，避免大 JSON 进入上下文。下方展示图表可视化技能当前启用状态。</p>
